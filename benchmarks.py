...
# Shifted rastrigin # x is a vector
def F24(x):
    shifted_x= x-2
    dim = len(shifted_x)
    o = numpy.sum(shifted_x ** 2 - 10 * numpy.cos(2 * math.pi * shifted_x)) + 10 * dim # x^2 - 10cos(2pix) + (10*dim)
    return o

def getFunctionDetails(a, dim):
    # [name, lb, ub, dim]
    param = {
        "F1": ["F1", -100, 100, dim],#30
        "F2": ["F2", -10, 10, dim],#30
        "F3": ["F3", -100, 100, dim],#30
        "F4": ["F4", -100, 100, dim],#30
        "F5": ["F5", -30, 30, dim],#30
        "F6": ["F6", -100, 100, dim], #30
        "F7": ["F7", -1.28, 1.28, dim], #30
        "F8": ["F8", -500, 500, dim], #30
        "F9": ["F9", -5.12, 5.12, dim], #30 # rastrigin
        "F10": ["F10", -32, 32, dim], #30
        "F11": ["F11", -600, 600, dim], #30
        "F12": ["F12", -50, 50, dim], #30
        "F13": ["F13", -50, 50, dim], #30
        "F14": ["F14", -65.536, 65.536, dim], #2
        "F15": ["F15", -5, 5, dim], #4
        "F16": ["F16", -5, 5, dim], #2
        "F17": ["F17", -5, 15, dim], #2
        "F18": ["F18", -2, 2, dim], #2
        ...
        "F22": ["F22", 0, 10, dim], #4
        "F23": ["F23", 0, 10, dim], #4
        "F24": ["F24", -5.12, 5.12, dim], #30 # shifted rastrigin
    }
